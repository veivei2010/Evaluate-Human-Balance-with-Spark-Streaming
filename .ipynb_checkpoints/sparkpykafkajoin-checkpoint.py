...
zSetEntriesRedisServerDF=spark.sql("SELECT zSetEntries[0].element AS encodedCustomer FROM RedisSortedSet")
...
zDecodedRedisServerDF=zSetEntriesRedisServerDF.withColumn("customer", unbase64(zSetEntriesRedisServerDF.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords

zDecodedRedisServerDF.withColumn("customer", from_json("customer", CustomerSchema))\
                             .select(col('customer.*'))\
...
CustomerStreamingDF=customerRiskStreamingDF \
...
# CustomerStreamingDF.writeStream.outputMode("append").format("console").start().awaitTermination()  - for test the result
...
CustomerStreamingDF.selectExpr("to_json(struct(*)) AS value")\
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("FailOnDataLoss" , "false") \
    .option("topic", "risk-score-output")\
